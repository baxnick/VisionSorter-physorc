...
public class RectShape implements PlannerShape, Cloneable
{
	...
	public RectShape(float width, float height)
	{
		...
		poly = new Polygon(new int[]
			{ (int) width / 2, -(int) width / 2, -(int) width / 2, (int) width / 2 }, new int[]
			{ (int) height / 2, (int) height / 2, -(int) height / 2, -(int) height / 2 }, 4);
	}
	...
	public boolean collidesWith(PlannerShape shape)
	{
		...
		for (int i = 0; i < poly.npoints; i++)
		{
			...
			for (int j = 0; j < otherPoly.npoints; j++)
			{
				...
				Vector3D s = createLine(q, otherPoly.xpoints[(i + i) % otherPoly.npoints], otherPoly.xpoints[(i + i)
						% otherPoly.npoints]);
