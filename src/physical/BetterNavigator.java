...
/**
 * The BetterNavigator class uses dead reckoning to  keep track of its robot pose (the location in the plane
 ...
 * BetterNavigator can perform  three elementary movements  in a plane:  trvel in  a straignt line,
...
public class BetterNavigator {
   ...
   *             BetterNavigator. Otherwise the BetterNavigator needs to know detail it should not care about!
     */
    public BetterNavigator(float wheelDiameter, float trackWidth, TachoMotor leftMotor, TachoMotor rightMotor, boolean reverse) {
 ...
 */
  ...
  private Point adjustedPoint(float targetX, float targetY, float gotoAngle, int multiplier)
  {
	  ...
	  return new Point(
		      (float) (targetX + multiplier * tracked_point.y * Math.cos(heading)),
