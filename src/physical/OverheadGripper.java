...
public class OverheadGripper implements Gripper {
	...
	public void calibrate() {
		...
		if (config.reverseDirection) motor.forward();
		else motor.backward();
		...
		try
		{
			Thread.sleep((int) (1000 * 
					(config.calibrationArc / config.calibrationSpeed)));
		}
	...
	@Override
	public void grip() {
		if (state == GripperState.GRIPPING || state == GripperState.GRIPPED)
			return;
		
		if (state == GripperState.RELEASING || state == GripperState.CALIBRATING)
			return;
	...
	@Override
	public void release() {
		if (state == GripperState.RELEASING || state == GripperState.RELEASED)
			return;
		
		if (state == GripperState.GRIPPING || state == GripperState.CALIBRATING)
			return;
